...
char keypad_scan() {
    for (uint8_t col = 0; col < 4; col++) {
        KEYPAD_PORT = ~(1 << col);
        _delay_us(1); // Reduced from 10us to 1us for faster scanning
...
void seven_seg_init() {
    SEVEN_SEG_DDR = 0xFF; // Set all pins as output
    SEVEN_SEG_PORT = 0x00; // Turn off all segments initially
}

void seven_seg_display(uint8_t num) {
    if (num < 10) {
        SEVEN_SEG_PORT = seven_seg_patterns[num];
    } else {
        SEVEN_SEG_PORT = 0x00; // Turn off all segments if number is out of range
    }
}
...
void init_game() {
    ...
    seven_seg_display(lives); // Display initial lives
}
...
void move_frog(uint8_t direction) {
    ...
    if (board[frog_y][frog_x] == OBSTACLE_UP || board[frog_y][frog_x] == OBSTACLE_DOWN) {
        lives--;
        seven_seg_display(lives); // Update seven-segment display when lives change
        if (lives == 0) return;
        frog_x = 0;
        frog_y = 1;
    }
...
int main() {
    ...
    while (1) {
        ...
        while (lives > 0 && frog_x < BOARD_WIDTH - 1) {
            ...
            if (key != 0) {
                switch(key) {
                    case '8': move_frog(0); break; // Up
                    case '2': move_frog(1); break; // Down
                    case '4': move_frog(2); break; // Left
                    case '6': move_frog(3); break; // Right
                }
            }
        ...
        seven_seg_display(lives); // Display final lives count
        // _delay_ms(2000);
    }
